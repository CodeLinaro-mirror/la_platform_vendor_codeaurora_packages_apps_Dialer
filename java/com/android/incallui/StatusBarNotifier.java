  ...
  private CharSequence getContentString(DialerCall call, @UserType long userType) {
    ...
    if (isIncomingOrWaiting) {
      if (call.isSpam()) {
        ...
        message = context.getString(R.string.notification_incoming_conf_call);
      ...
      if(!VideoProfile.isBidirectional(requestedVideoState)) {
        if(VideoProfile.isTransmissionEnabled(requestedVideoState)) {
          message = context.getString(R.string.notification_requesting_video_tx_call);
        } else if(VideoProfile.isReceptionEnabled(requestedVideoState)) {
          message = context.getString(R.string.notification_requesting_video_rx_call);
        }
      }
    }
    return message;
